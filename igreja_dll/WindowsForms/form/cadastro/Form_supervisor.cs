...
namespace igreja2.form.cadastro
{
    public partial class Form_supervisor : Form
    {
        ...
        Cargo_Supervisor_Treinamento sup_t = new Cargo_Supervisor_Treinamento();

        private void button1_Click(object sender, EventArgs e)
        {
            Cargo_Supervisor sup = new Cargo_Supervisor();
            Cargo_Supervisor_Treinamento sup_trei = new Cargo_Supervisor_Treinamento();
            sup.Supervisorid = int.Parse(text_id_supervisor.Text);
            sup.salvar();
            sup_trei.Supervisortreinamentoid = int.Parse(text_id_supervisor_treinamento.Text);
            sup_trei.salvar();
        ...
        private void text_id_supervisor_TextChanged(object sender, EventArgs e)
        {
            if (text_id_supervisor.Text != "")
            {
                try
                {
                    ...
                    pes.Id = int.Parse(lbl_id.Text);
                    pes = pes.recuperar(pes.Id);
                    text_nome_sup.Text = pes.Nome;
                }
        ...
        private void text_id_supervisor_treinamento_TextChanged(object sender, EventArgs e)
        {
            if (text_id_supervisor_treinamento.Text != "")
            {
                try
                {
                    sup_t.Supervisortreinamentoid = int.Parse(text_id_supervisor_treinamento.Text);
                    Form_supervisor_Load(text_id_supervisor_treinamento.Text, e);                
                    Pessoa pes = new Pessoa();
                    lbl_id.Text = text_id_supervisor_treinamento.Text;
                    pes.Id = int.Parse(lbl_id.Text);
                    pes = pes.recuperar(pes.Id);
                    text_nome_sup_treinamento.Text = pes.Nome;
                }

                catch (Exception ex)
                {
                    MessageBox.Show("Por favor digite um numero.");
                    sup_t.Supervisortreinamentoid = 0;
                    Form_supervisor_Load(text_id_supervisor_treinamento.Text, e);
        ...
        private void Form_supervisor_Load(object sender, EventArgs e)
        {
            ...
            if (sup.Supervisorid == 0 || sup_t.Supervisortreinamentoid == 0)
            {
        ...
        private void text_id_celula_TextChanged(object sender, EventArgs e)
        {
            if(text_id_celula.Text != "")
            {
                ...
                {                    
                    ...
                    cel.Celulaid = int.Parse(text_id_celula.Text);
                    cel = cel.recuperar(cel.Celulaid);
                    text_nome_celula.Text = cel.Cel_nome;
